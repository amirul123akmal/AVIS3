        ...
        <div class="container mx-auto bg-base-100 p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="col-span-3">
                    ...
                    <form action="{{ route('Update-TransportPost', ['id' => $transportation->transID]) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            @foreach ($vehicleType as $vehicle)
                            <div class="flex items-center">
                                <input type="radio" name="vehicleType" value="{{ $vehicle->vehicleID }}" class="radio radio-primary mr-2" {{ $vehicle->vehicleID == $transportation->vehicleID ? 'checked' : '' }} />
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="plateNumber" name="plateNumber" required class="w-full input input-bordered input-primary" value="{{ $transportation->vehiclePlateNumber }}" />
                        </div>
                        
                        <!-- Capacity Input -->
                        <div class="mb-4">
                            <label for="capacity" class="block text-sm font-medium text-gray-700">Capacity</label>
                            <input type="number" id="capacity" name="capacity" required class="w-full input input-bordered input-primary" value="{{ $transportation->vehicleCapacity }}" />
                        </div>
                        
                        <!-- Description Input -->
                        <div class="mb-4">
                            <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                            <textarea id="description" name="description" rows="4" class="w-full textarea textarea-bordered textarea-primary">{{ $transportation->vehicleDesc }}</textarea>
                        </div>
